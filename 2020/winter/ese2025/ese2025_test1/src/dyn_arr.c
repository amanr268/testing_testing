...
int comparo_fName(const void *pA, const void *pB)
{
	...
	stuDA_t stud_A = *((stuDA_t*) pA);
  	stuDA_t stud_B = *((stuDA_t*) pB);
  	char *fName_A = stud_A.data.fName;
  	char *fName_B = stud_B.data.fName;
...
int comparo_grade(const void *pA, const void *pB)
{
	...
	//	       NOTE: PLEASE COMMIT CHANGES ONCE YOUR DONE THIS FUNCTION
	stuDA_t stu_A = *((stuDA_t*) pA);
    	stuDA_t stu_B = *((stuDA_t*) pB);
    	float G_A = stu_A.data.grade;
    	float G_B = stu_B.data.grade;
